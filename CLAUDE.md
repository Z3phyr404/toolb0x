...
nginx-Konfiguration: `deploy/nginx-fotos.conf` - per `include` direkt aus dem
Repo-Checkout eingebunden, die Datei IST die Live-Config. Deploy-Ablauf bei
Änderungen daran: committen → auf dem Server `git pull` → `nginx -t &&
...
- **`changeMonth` muss bei Ladefehlern zurückspringen** (seit 2026-09-07): Sonst steht die Überschrift auf dem neuen Monat, während Tabelle, Kacheln und Donut die Zahlen des alten zeigen — eine neue Ausgabe landet dann im falschen Monat.
